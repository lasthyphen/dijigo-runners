...
import (
	...
	"github.com/lasthyphen/beacongo/vms/platformvm/validator"
...
func (lc *localNetwork) installCustomVMs(
	...
	baseWallet, djtxAssetID, testKeyAddr, err := setupWallet(ctx, httpRPCEp, pTXs)
	if err != nil {
		return nil, err
	}
	...
	zap.L().Info("base wallet DJTX balance",
		zap.String("address", testKeyAddr.String()),
		zap.Uint64("balance", balances[djtxAssetID]),
	)
...
func setupWallet(
...
) (baseWallet *refreshableWallet, djtxAssetID ids.ID, testKeyAddr ids.ShortID, err error) {
	...
	bal, ok := balances[djtxAssetID]
	if bal <= 1*units.Djtx || !ok {
		return nil, ids.Empty, ids.ShortEmpty, fmt.Errorf("not enough DJTX balance %v in the address %q", bal, testKeyAddr)
	}
	zap.L().Info("fetched base wallet DJTX balance",
		zap.String("http-rpc-endpoint", httpRPCEp),
		zap.String("address", testKeyAddr.String()),
		zap.Uint64("balance", bal),
	)

	return baseWallet, djtxAssetID, testKeyAddr, nil
}
...
func checkValidators(
	...
	// ref. https://docs.djtx.network/build/avalanchego-apis/p-chain/#platformgetcurrentvalidators
